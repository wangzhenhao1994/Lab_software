...
class tof_alignment_gui():
    def __init__(self, doc, running, delayer):
        ...
        self.boxcarAveragedNum = bokeh.models.widgets.TextInput(
            title='Boxcar averaged number', value='100')
        if longstage:
            self.stagePos = bokeh.models.widgets.TextInput(
                title='Stage Position [mm]', value='-102.50616')
        else:
            self.stagePos = bokeh.models.widgets.TextInput(
...
class tof_measurement_gui(tof_alignment_gui):
    def __init__(self, doc, running, delayer, daq):
        super().__init__(doc, running, delayer)
        ...
        if longstage:
            ...
            title='Zero Delay [mm]', value='-102.506')
        else:
            self.zeroDelInput = bokeh.models.widgets.TextInput(
        ...
        # arrange items
        self.inputs = bokeh.layouts.Column(
            self.startBtn, self.integrationInput, self.boxcarAveragedNum, self.zeroDelInput,
        ...
        self.layout = bokeh.layouts.grid(
            ...
            bokeh.layouts.column(
                self.sumAutocorrelationPlot.element, self.linePlot_channel0.element, self.linePlot_channel1.element,
                 self.linePlot_channel2.element, self.linePlot_channel3.element, self.linePlot_channel4.element)],
            ncols=4)
    ...
    def start(self):
        ...
        # in case a different measurement is running, do nothing
            ...
            #SR245 setup
            self.daq.sendcmd('MR')
            self.daq.setAllPortsRead()
            self.daq.setAsyMode()
            readEveryNtrigger = int(float(self.boxcarAveragedNum.value))
            self.daq.setTrigEveryN_B1(readEveryNtrigger) # read from boxcar every n laser trigger
            self.readNTime = math.ceil(self.integration*3000/readEveryNtrigger)
            # delay vector setup
            ...
            self.measurement = measurement.measurement(
                inputs=[[self.readNTime, pos, self.readNTime] for pos in self.stage_values],
                sequence=[
                    measurement.single_input_sequence_function(self.daq.readSSBit),
                    ...
                    measurement.single_input_sequence_function(self.daq.getSSBit_response),
                    ],

                update=measurement.bokeh_update_function(
                    self.update, self.doc),
                init=partial(self.daq.sendcmd,command='MR'),
