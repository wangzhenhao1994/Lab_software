...
class SMC100(object):
  ...
  def get_position_um(self):
    return float(self.get_position_mm()*1000)
  
  def get_speed_mm(self):
    speed_mm = float(self.sendcmd('VA', '?', expect_response=True, retry=10))
    return speed_mm
  ...
  def move_absolute_mm(self, position_mm, waitStop=True):
    ...
    If waitStop is True then this method returns when the move is completed.
    """
    position_mm = np.round(position_mm,5)
  ...
  def move_absolute_mm_2(self, position_mm, waitStop=True):
    """
    Moves the stage to the given absolute position given in mm.
    """
    position_mm = np.round(position_mm,5)
  ...
  def move_absolute_um(self, position_um, **kwargs):
    ...
    If waitStop is True then this method returns when the move is completed.
    """
    pos_mm = floor(position_um*2)/2/1000
    return self.move_absolute_mm(pos_mm, **kwargs)

  def set_speed_mm(self, speed):
    self.sendcmd('VA', speed)
    print('The speed of the stage is ' + str(self.get_speed_mm()) + ' mm.')
...
# Tests #####################################################################
  ...
  def test_configure():
    smc100 = SMC100(1, 'COM22', silent=False)
...
#test_general()
  ...
  stage.stop()
  stage.set_speed_mm(20)
  #stage.reset_and_configure()
  #for i in np.arange(11.3,12,0.01 ):
  #stage.move_absolute_mm(11.411+600*1e-15*3e8*1e3/2)
  print(np.round(stage.get_position_mm(),5))
  #for i in range(100):
  stage.move_relative_mm(-100)
  #stage.move_absolute_mm(-102.40242) #piezo 4
  #print(np.round(stage.get_position_mm(),5))
  #stage.sendcmd('PW', 1)
  #stage.sendcmd('BA', 2)
  #stage.sendcmd('PW', 0)
  #print(stage.sendcmd('BA','?',expect_response=True,retry=10))
  stage._port.close
