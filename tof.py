...
calclator = Calibration_mass()#def __init__(self, mass=[18.01528,31.99880], pixel=[2659,4575])
[k,b]=calclator.cal_k_b()

stageTag = 'piezoStage'#'piezoStage','longStage','mixed'
testing = False
if not testing:
    # for the lab
    from small_lab_gui.digitizers.digitizer_fast_mcs6a import mcs6a
    if stageTag=='longStage':
        from smc100pp import SMC100
    elif stageTag=='piezoStage':
    ...
    def __init__(self, doc, running, tof_digitizer, delayer):
        ...
        if stageTag=='longStage':
            self.stagePos = bokeh.models.widgets.TextInput(
                title='Stage Position [mm]', value='11.3503')
        else:
            self.stagePos = bokeh.models.widgets.TextInput(
        ...
        self.layout = bokeh.layouts.grid(
            [self.inputs,
             self.linePlot.element,bokeh.layouts.row(bokeh.layouts.column(width=500),
              self.totalCount, width=800)],
               ncols=4)
        ...
        if not (data is None):
            ...
            if np.abs(curspec - self.lastspectrum).sum():
                ...
                self.linePlot.update(
                    num=0, x=self.pixel2mass(np.arange(0, self.spectrum.size)), y=self.spectrum)
...
class tof_measurement_gui(tof_alignment_gui):
    def __init__(self, doc, running, tof_digitizer, delayer, logbook = None):
        ...
        if stageTag=='longStage':
            ...
            self.zeroDelInput_longStage = bokeh.models.widgets.TextInput(
            title='Zero Delay [mm]', value='11.3503')
            self.zeroDelInput_piezoStage = bokeh.models.widgets.TextInput(
            title='Zero Delay [um]', value='50.')
        ...
        # in case a different measurement is running, do nothing
            ...
            if self.scanTableBtn.active:
                ...
                if stageTag=='longStage':
                    ...
                    print('The step size is '+ str(stepSizeInput)+'\n')
                    self.delays = np.arange(startDelInput,stopDelInput,stepSizeInput)
                    #print(startDelInput,stopDelInput, stepSizeInput)
                    #print(self.delays)
                else:
    ...
    def update(self, data):
        ...
        sweeps = np.array([d[1]['sweeps'] for d in data],dtype=int)
        for i in sweeps:
            if i==np.int32(self.integrationInput.value)*1000:
                pass
            else:
                print("Sweeps is wrong!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        im_data = np.array([d[1]['data'][0] for d in data])
        im_data = im_data/sweeps[:,None]*int(self.integrationInput.value)*1000
        ...
        try:
            ...
            self.imagePlot.update(
                num=0, x=curdelays, y=self.pixel2mass(pixelMin,pixelMax,1), z=plot_data)
            self.autocorrelationPlot.update(
                num=0, x=curdelays, y=np.sum(plot_data,0))
...
class tof_session_handler(bgh.bokeh_gui_session_handler):
    def open_session(self, doc):
        ...
        if stageTag=='longStage':
            ...
            stage = [SMC100(1, 'COM22', silent=True), Stage(pidevice)]
