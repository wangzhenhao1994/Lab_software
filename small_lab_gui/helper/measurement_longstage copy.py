...
class measurement(Thread):
    ...
    def __init__(self, inputs=None, sequence=None, init=None,
        ...
        self.scanNum = 0 # the condition of the pump/probe only measurement, it's defined in the main program
        # output list
        self.output = []
        ...
        #due to the in-motion trigger of the long stage, the output need to be readout inbetween a measurement sequence
        self.retlist = []
                ...
                if self.inputs is not None:
                    ...
                    for inplist in tqdm(self.inputs):
                        ...
                        if self.stop_event.is_set():
                            break
                        if self.scanNum%2!=0 or self.scanNum==0:# has to be an even number
                            ###############################################################################
                            # this list holds all sub step results in a step
                            self.retlist = []
                            # every entry in sequence is a sub step
                            for step, inp in zip(self.sequence[0:6], inplist[0:6]):
                                # break condition
                                if self.stop_event.is_set():
                                    break
                            ...
                            if not self.stop_event.is_set():
                                self.update(self.retlist.copy())
                            self.scanNum = self.scanNum+1
                            ##################################################################################################
                            print('Now the stage go back to the start position!')
                            self.retlist = []
                            # every entry in sequence is a sub step
                            for step, inp in zip(self.sequence[6:11], inplist[6:11]):
                                # break condition
                                if self.stop_event.is_set():
                                    break
                                # execute sub step and save result
                            ...
                            print('Now measure the pump only spectra!')
                            self.retlist = []
                            # every entry in sequence is a sub step
                            for step, inp in zip(self.sequence[11:17], inplist[11:17]):
                                # break condition
                                if self.stop_event.is_set():
                                    break
                                # execute sub step and save result
                            ...
                            if not self.stop_event.is_set():
                                self.update(self.retlist.copy())
                            #------------------------------------------
                            print('Now measure the probe only spectra!')
                            self.retlist = []
                            # every entry in sequence is a sub step
                            for step, inp in zip(self.sequence[17:24], inplist[17:24]):
                                # break condition
                                if self.stop_event.is_set():
                                    break
                                # execute sub step and save result
