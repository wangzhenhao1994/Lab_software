...
class plot_1d(plot):
    ...
    def line(self, legend='', line_color='blue'):
        ...
        line = self.plot.line(
            ...
            legend_label=legend, line_color=line_color)
        # line = self.plot.step(
        #    x='x', y='y', source=source.source,
...
class plot_2d(plot):
    def __init__(self, title='', height=256, width=1024, max_refresh=0.05, mm=False):
        ...
        self.autoscaleYBtn = bokeh.models.Toggle(
            label='Autoscale Y', active=True, width=int(np.floor(width/6)))
        self.y_axis_scale =  bokeh.models.Toggle(
            label='Log Scale', active=True, width=int(np.floor(width/6)))
        # replot button
        self.replotBtn = bokeh.models.Button(
            label='Replot', width=int(np.floor(width/6)))
        self.replotBtn.on_click(lambda inp1: self.update_all())
        ...
        self.element = bokeh.layouts.column(
            ...
            bokeh.layouts.row(
                bokeh.layouts.Column(self.autoscaleXBtn),
                bokeh.layouts.Column(self.autoscaleYBtn),
                bokeh.layouts.Column(self.y_axis_scale),
                bokeh.layouts.Column(self.replotBtn)))
    ...
    def y_axis_log(self, y, checkBtn=True):
        if self.y_axis_scale.active or (not checkBtn):
            return np.log10(y+0.1)
        else:
            return y
...
class plot_false_color(plot_2d):
    def __init__(self, title='', height=256, width=1024, max_refresh=0.2):
        ...
        #log scale    
        self.z_axis_scale =  bokeh.models.Toggle(
            label='Log Scale', active=True, width=int(np.floor(width/6)))
        # replot button
        self.replotBtn = bokeh.models.Button(
            label='Replot', width=int(np.floor(width/6)))
        ...
        self.element = bokeh.layouts.column(
            ...
            bokeh.layouts.row(
                ...
                bokeh.layouts.Column(self.z_axis_scale),
                bokeh.layouts.Column(self.replotBtn)))
    ...
    def z_axis_log(self, z, checkBtn=True):
        if self.z_axis_scale.active or (not checkBtn):
            return np.log10(z+0.1)
        else:
            return z
...
def reduce_x_1d_c_no_y(x_scale, y, goal_length_x, x_start, x_end):
    y = y[int(np.floor(x_start/x_scale)):int(np.ceil(x_end/x_scale))]
    ...
    if (old_x_size > goal_length_x):
        ...
        x_disp = np.linspace(x_start, x_end, len(y_disp))
        y_disp = y_disp/factor
    else:
        x_disp = np.linspace(x_start, x_end, len(y))
        y_disp = y
    return x_disp, y_disp
