...
class Andor:
    def __init__(self):
        ...
        if platform.system() == "Windows":
            if platform.architecture()[0] == "32bit":
                self.dll = WinDLL("C:\\Program Files\\Andor SOLIS\\atmcd32d")
            else:
                self.dll = WinDLL("C:/Program Files/Andor SDK/atmcd64d")
        # for Linux
    ...
    def GetAcquiredData(self,imageArray):
        ...
        cimageArray = c_int * int(dim)
        cimage = cimageArray()
        error = self.dll.GetAcquiredData(pointer(cimage), int(dim))
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
    ...
    def GetMostRecentImage(self,imageArray):
        if not (self.AcquisitionMode==5):
            print('most recent image is for acquisition mode 5')
            return
        if (self.ReadMode==4):
            dim = self.width * self.height / self.hbin / self.vbin
        elif (self.ReadMode==3 or self.ReadMode==0):
        ...
        cimageArray = c_int * int(dim)
        cimage = cimageArray()
        error = self.dll.GetMostRecentImage(pointer(cimage), int(dim))
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
