# wrapper to use fast comtechs's mcs6a in python
# probably not complete

from ctypes import cdll, c_int, c_uint, c_ubyte, c_char, c_long, c_ulong, c_double, c_void_p, Structure, POINTER, byref
...
class mcs6aDll():
    def __init__(self, path=''):
        #try:
        self.dll = cdll.LoadLibrary(path + 'DMCS6.dll')
        #except Exception:
        #    print('could not find ' + path + 'DMCS6.dll')
...
# Get Spectra Settings stored in the DLL
    def GetSettingData(self, nDisplay):
        ...
        return {'err': IMPAGETSETTING(byref(Setting), nDisplay),
                'Setting': Setting}
...
# Get the Status
    def GetStatusData(self, nDisplay):
        ...
        return {'err': IMPAGETSTATUS(byref(Status), nDisplay),
                'Status': Status}

# typedef int APIENTRY GetData(ACQDATA FAR *Data, int nDisplay);// Get Data pointers
    def GetData(self, nDisplay):
        Data = self.ACQDATA()
        IMPAGETDATA = self.dll.GetData
        IMPAGETDATA.restype = c_int
        IMPAGETDATA.argtypes = [POINTER(self.ACQDATA), c_int]
        return {'err': IMPAGETDATA(byref(Data), nDisplay),
                'Data': Data}
...
# spectrumlength is defined by the range of the scan
    def LVGetDat(self, nDisplay):
        ...
        data = np.frombuffer(datp, dtype=np.dtype(c_ulong))
...
# typedef UINT (WINAPI *IMPASERVEXEC) (HWND ClientWnd);
# Register client at server MCS6.EXE
...
# typedef int (WINAPI *IMPANEWSTATUS) (int nDev);
...
# typedef int (WINAPI *IMPADACOUT) (int value);
...
# typedef VOID (WINAPI *IMPASTART) (int nSystem);
# Start
...
# typedef VOID (WINAPI *IMPAHALT) (int nSystem);
# Halt
...
# typedef VOID (WINAPI *IMPACONTINUE) (int nSystem);
# Continue
...
# typedef VOID (WINAPI *IMPAERASE) (int nSystem);
# Erase spectrum
