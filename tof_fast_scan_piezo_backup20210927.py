...
calclator = Calibration_mass(mass=[2,17], pixel=[384,1276])#def __init__(self, mass=[18.01528,31.99880], pixel=[2659,4575])
...
class tof_measurement_gui(tof_alignment_gui):
    def __init__(self, doc, running, tof_digitizer, delayer, logbook = None):
        ...
        self.sweepTimeInput = bokeh.models.widgets.TextInput(
            title='Integration time [s]', value='10') #seconds #125 is the value before Sep. 120 is the value since 8th Sep.
        self.numCycle = 1 # number of cycle after initiation of the wave generator
        self.numSweeps = bokeh.models.widgets.TextInput(
            title='Sweep Number', value='200') # number of sweeps
        self.ppOnlyNum = bokeh.models.widgets.TextInput(
            title='Pump/Probe-Only number', value=str(100000000))

        self.averageNum = bokeh.models.widgets.TextInput(
            title='Boxcar Average', value='25') # number of average
        ...
        self.inputs = bokeh.layouts.Column(
            ...
            self.sweepTimeInput, #self.ppOnlyNum,
        ...
        self.scanNum = 0
        self.cycles = 1200 #72000 correspongind t0 120.5s of measurement
        self.range = 24000#1536 # 1024 for H2O, 1536 for 2-Proponal
    ...
    def start(self):
        ...
        # in case a different measurement is running, do nothing
            ...
            self.sweepTime = math.ceil(int(self.sweepTimeInput.value)/1.6)*1.6+1.5
            # measurement thread
            self.measurement = measurement.measurement(
                inputs=[[None,None,None,None,None] for i in range(int(self.totalScanNum))],
                sequence=[
                    ...
                    measurement.sleep_function(self.sweepTime),
...
class tof_session_handler(bgh.bokeh_gui_session_handler):
    def open_session(self, doc):
        ...
        # hardware
        digitizer = mcs6a(cycles=1200, swpreset=25, sweepmode='1e2004')
        #digitizer = mcs6a(cycles=1200, swpreset=25, sweepmode='1e0204')
        if longstage:
