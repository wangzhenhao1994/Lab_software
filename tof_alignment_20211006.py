...
class tof_measurement_gui(tof_alignment_gui):
    ...
    def update(self, data):
        # loop steps
        self.curdelays = np.array([d[0] for d in data])#-float(self.zeroDelInput.value)
        print(self.curdelays)
        ...
        try:
            ...
            self.imagePlot.update(
                num=0, x=self.curdelays, y=self.pixel2mass(np.arange(plot_data.shape[0])+pixelMin), z=plot_data)
            self.autocorrelationPlot.update(
                num=0, x=self.curdelays, y=np.sum(plot_data,0))
            if len(self.curdelays) >int(self.dcRangeInput.value)+3:
                fd=FFT_ionS2(self.curdelays, plot_data, [float(self.massMinInput.value),float(self.massMaxInput.value)], int(self.dcRangeInput.value))
                fd.dataProcess()
                [fre,Y] = fd.fftS["mass"]
                self.fftPlot.update(num=0, x=fre, y=Y)
